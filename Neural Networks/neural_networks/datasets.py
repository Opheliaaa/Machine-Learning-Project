...
class Dataset:
    def __init__(
        ...
        training_set = (training_set - np.mean(training_set, axis=0)) / np.sqrt(np.var(training_set, axis=0))
        if validation_set is not None:
            validation_set = (validation_set - np.mean(validation_set, axis=0)) / np.sqrt(
                np.var(validation_set, axis=0))
        if test_set is not None:
            test_set = (test_set - np.mean(test_set, axis=0)) / np.sqrt(np.var(test_set, axis=0))
        self.batch_size = batch_size
