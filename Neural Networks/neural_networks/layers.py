...
class FullyConnected(Layer):
    ...
    def _init_parameters(self, X_shape: Tuple[int]) -> None:
        ...
        W = self.init_weights(shape=(self.n_in,self.n_out))
        b = np.zeros(self.n_out)
        self.parameters = OrderedDict({"W": W, "b": b})
        self.cache: OrderedDict = OrderedDict()
        # what do you need cache for backprop?
        self.gradients: OrderedDict = OrderedDict({"W": np.zeros([self.n_in,self.n_out]), "b": np.zeros(b.shape)})
...
class Elman(Layer):
    ...
    def _init_parameters(self, X_shape: Tuple[int]) -> None:
        ...
        self.gradients: OrderedDict = OrderedDict({"W": np.zeros((self.n_in, self.n_out)),
                                                   "U": np.zeros((self.n_out, self.n_out)),
                                                   "b": np.zeros(self.n_out)})
        self._init_cache(X_shape)
    ...
    def _init_cache(self, X_shape: Tuple[int]) -> None:
        ...
        """
        s0 = np.zeros((X_shape[0], self.n_out))
        self.cache = OrderedDict({"s": [s0], 'W':self.parameters['W'], 'U':self.parameters['U'] ,
                                  'b':self.parameters['b'], 'X':[], 'Z':[]
                                  })  # THIS IS INCOMPLETE
    def forward_step(self, X: np.ndarray) -> np.ndarray:
        ...
        z = np.dot(X, self.parameters['W'])+np.dot(self.cache['s'][-1],self.parameters['U'])+self.parameters['b']
    ...
    def backward(self, dLdY: np.ndarray) -> List[np.ndarray]:
        ...
        for t in range(len(X)-1, -1, -1):
            dLdY = self.activation.backward(Z[t], dLdY)
            dLdX.append(np.dot(dLdY, W.T))
            dLdW += np.dot(X[t].T, dLdY)
            dLdU += np.dot(s[t].T, dLdY)
            dLdb += np.sum(dLdY,axis=0)
            dLdY = np.dot(dLdY, U.T)
        self.gradients["W"] = dLdW
...
class Conv2D(Layer):
    ...
    def _init_parameters(self, X_shape: Tuple[int]) -> None:
        ...
        # (kernel_shape[0],kernel_shape[1],self.n_in,self.n_out,)
        self.n_in=X_shape[3]
    ...
    def forward(self, X: np.ndarray) -> np.ndarray:
        ...
        out_rows=in_rows+p[0]+p[1]-(kernel_height-1)
        out_cols=in_cols+p[2]+p[3]-(kernel_width-1)
        pad_X=np.pad(X,pad_width=((0,0),(p[0],p[1]),(p[2],p[3]),(0,0)),mode='constant',constant_values=0)
            ...
            for c in range(out_channels):
                for i in range(out_rows):
                    for j in range(out_cols):
                        Z[n,i,j,c]=np.sum(W[:,:,:,c]*pad_X[n,i:i+kernel_height,j:j+kernel_width,:])+b[c]
        # cache any values required for backprop
        self.cache["Z"] = Z
        self.cache["X"] = X
        self.cache['pad_X']=pad_X
        ### END YOUR CODE ###
    ...
    def backward(self, dLdY: np.ndarray) -> np.ndarray:
        ...
        dLdW=np.zeros(W.shape)
        dLdB=np.zeros(b.shape)
        dLdX_pad=np.zeros(pad_X.shape)
        dLdZ = self.activation.backward(Z, dLdY)
        for n in range(n_examples):
            for c in range(out_channels):
                dLdB[c]+=np.sum(dLdY[n,:,:,c])
                for i in range(out_rows):
                    for j in range(out_cols):
                        dLdX_pad[n,i:(i+kernel_height),j:(j+kernel_width),:] += np.dot(dLdZ[n,i,j,c],W[:,:,:,c])
                        dLdW[:,:,:,c]+=pad_X[n,i:(i+kernel_height),j:(j+kernel_width),:]*dLdZ[n,i,j,c]
        dLdX=dLdX_pad[:,p[0]:-p[1],p[2]:-p[3],:]
        self.gradients["W"]=dLdW
        self.gradients["b"]=dLdB
        return dLdX
